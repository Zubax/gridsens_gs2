...
carriage return (\verb|LF|) plus line feed (\verb|LF|) character sequence (\verb|\r\n|).
...
\begin{ZubaxSimpleTable}{Some of the standard NMEA talker ID values}{|l X|}
    Talker ID & Purpose \\
...
Some of the standard sentences are documented in the following section \ref{nmea_standard_sentences}.

After the sentence ID follows a comma (\verb|,|) followed by a list of comma-separated parameters.
The list of parameters is terminated by an asterisk (\verb|*|),
after which there is a checksum.

The checksum is computed <TODO>.
