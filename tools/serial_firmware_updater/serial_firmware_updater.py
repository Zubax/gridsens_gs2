...
class CommandInterface:
    ...
    def _wait_for_ask(self, info = ""):
        ...
        try:
            ...
            raise CmdException("Can't read port or timeout: %s" % ex)
        else:
            if ask == 0x79:
                # ACK
                return 1
...
def run(cmd, binary):
    ...
    try:
        ...
        data = map(lambda c: ord(c), open(binary, 'rb').read())
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print 'Usage:\n\t%s <serial_port> <binary_file>' % sys.argv[0]
        print 'Example:\n\t%s /dev/ttyUSB0 ../../firmware/build/*.bin' % sys.argv[0]
        exit(1)
    ...
    cmd.open(port, baudrate)

    cmd.sp.write('\r\n\r\nbootloader\r\n')
    time.sleep(0.5)
    cmd.sp.read(999)     # Flush read queue
